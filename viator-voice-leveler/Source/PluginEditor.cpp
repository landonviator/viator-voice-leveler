...
ViatorvoicelevelerAudioProcessorEditor::ViatorvoicelevelerAudioProcessorEditor (ViatorvoicelevelerAudioProcessor& p)
...
{    
    ...
    _levelMeter.setSliderStyle(juce::Slider::SliderStyle::LinearVertical);
    _levelMeter.setTextBoxStyle(juce::Slider::TextBoxBelow, true, 64, 64);
    _levelMeter.setRange(-60.0, 15.0, 0.1);
...
ViatorvoicelevelerAudioProcessorEditor::~ViatorvoicelevelerAudioProcessorEditor()
{
    stopTimer();
}
...
void ViatorvoicelevelerAudioProcessorEditor::resized()
{
    // vars
...
#pragma mark Window
void ViatorvoicelevelerAudioProcessorEditor::setWindowSizeLogic()
{
...
void ViatorvoicelevelerAudioProcessorEditor::setBackgroundStyle(juce::Graphics &g)
{
...
void ViatorvoicelevelerAudioProcessorEditor::setSettingsState(bool isActive)
{
    _settingsComp.setVisible(isActive);
    _settingsComp.setEnabled(isActive);
}

void ViatorvoicelevelerAudioProcessorEditor::changeListenerCallback(juce::ChangeBroadcaster *source)
{
    // high contrast
...
#pragma mark Dials
void ViatorvoicelevelerAudioProcessorEditor::updateSliderColors()
{
    for (auto& dial : _pluginDials)
    {
...
void ViatorvoicelevelerAudioProcessorEditor::initDialProps()
{
    // plugin
    for (int i = 0; i < _numPluginDials; i++)
    {
...
void ViatorvoicelevelerAudioProcessorEditor::initTooltipLabel()
{
    _tooltipLabel.setText("", juce::dontSendNotification);
    addAndMakeVisible(_tooltipLabel);
}

void ViatorvoicelevelerAudioProcessorEditor::setTooltipText(const juce::String &tooltip)
{
    _tooltipLabel.setText(tooltip, juce::dontSendNotification);
}

#pragma mark Tooltips
void ViatorvoicelevelerAudioProcessorEditor::mouseEnter(const juce::MouseEvent &event)
{
...
void ViatorvoicelevelerAudioProcessorEditor::mouseExit(const juce::MouseEvent &event)
{
    for (int i = 0; i < _pluginDials.size(); ++i)
    {
        if (event.eventComponent == &_pluginDials[i]->getSlider())
